...
#' 
#' The function carries out the Reference Validation Procedure (RVP), described in Brill et. al. (2019), appendix A. The input to the function includes a count matrix of taxa selected as a reference set (rows represent samples), and a vector Y of groups labeling. The function first rarefies all samples to constant depth, then tests for multivariate equality of distributions across the differet study groups.
#' 
#' @details 
#' Rarefying samples to equal depth is mandatory, see Brill et. al. (2019) Appendix A for a complete discussion of the test and assumption. The multivariate K-sample tests performed include the \code{hhg.test.k.sample} (Heller et. al. 2019) from package \code{HHG}, The \code{disco} test (Székely et. al. 2007 ) from package \code{energy} and the Permanova test (Anderson et, al. 2001), available through the function \code{adonis} in the \code{vegan} package.
#' All three tests require a distance (or dissimilarity) metric between samples. All three tests are evaluated with both the L1,L2 and Bray-Curtis dissimilartiy metrics.
#' This function supports categorical phenotypes. For continous and multivariate phenotypes, a different procedure is required, making use of multivariate tests of independencesuch as \code{hhg.test} (from packge \code{HHG}), and \code{dcov.test} (from package \code{energy}) are better suited.
...
#' @param verbose Should messages be printed to console, for each test computed.
#' 
...
#' \item{p.value.permanova_L1}{ - P-value for the permanova K sample test, with the L1 distance metric}
#' \item{p.value.permanova_BC}{ - P-value for the permanova K sample test, with the Bray-Curtis dissimilarity metric}
#' }
#' 
#' @export
#'
